...
module Client =
    type Task =
        ...
        static member Toggle task _ = { task with State = not task.State }
    ...
    type State =
        ...
        static member Default =
            { Input = ""
              Tasks = [ { Name = "buy milk"; State = false } ] }
    ...
    type Main(props) as this =
        ...
        member private this.AddTask() =
            if this.State.Input.Length > 0
               && not (List.exists (fun task -> task.Name = this.State.Input) this.State.Tasks) then
                { this.State with
                      Input = ""
                      Tasks =
        ...
        member private this.ToggleTask(task) =
            { this.State with
                  Tasks =
                      this.State.Tasks
                      |> List.map (fun x -> if x.Name = task.Name then { task with State = not task.State } else x) }
            |> this.SetState

        override this.Render() =
            MUI.Paper [ attr.className this.Props.Classes.["root"] ] [
                MUI.Button [ attr.variant MUI.ButtonVariant.Contained
                             attr.fullWidth true
                             attr.color MUI.Color.Secondary
                             on.click (fun _ -> this.ClearCompleted()) ] [
                    text "Clear completed tasks"
                ]
                MUI.List [ attr.className this.Props.Classes.["list"]
                           attr.subheader (MUI.ListSubheader [] [ text "MyTasks" ]) ] [
                    for task in this.State.Tasks ->
                        MUI.ListItem [ attr.``component`` MUI.ButtonBase
                                       attr.button true
                ...
                MUI.TextField [ attr.fullWidth true
    ...
    let MyTheme =
        MUI.Theme(Palette = MUI.Palette(Primary = MUI.Colors.Green, Type = MUI.Dark))

    let MyStyles (theme: MUI.Theme) =
        New [ "root"
              => New [ "marginTop" => theme.Spacing.Unit
